...
export async function generateMetadata({ params }) {
  ...
  const title = `${intent.name} — ${intent.content_mode === 'recommendations' ? 'recommended resources' : 'guide and resources'}`;
  const description =
      ...
      <article data-guide-publication={intent.publication_id || undefined} className="bg-white border border-line rounded-xl overflow-hidden shadow-sm">
        <div className="p-5 sm:p-8 lg:p-10">
          ...
          <nav aria-label="In this guide" className="mb-8 flex max-w-3xl flex-wrap items-center gap-x-4 gap-y-2 rounded-lg border border-line px-4 py-3 text-sm">
            ...
            {slug === 'calculate-percentage' && <a href="#calculator" className="font-bold underline">Use calculator</a>}
          ...
          {slug === 'calculate-percentage' && <Calculator initialOperation="percent" />}

          <div className={recommendationsOnly || !liveRecs.length ? "flex flex-col gap-8" : "grid items-start gap-10 lg:grid-cols-[minmax(0,1fr)_22rem]"}>
            <div className={recommendationsOnly ? "order-2 min-w-0 w-full" : "min-w-0"}>
              {!recommendationsOnly && intent.steps?.length > 0 && <section id="instructions" className="scroll-mt-6 mb-10">
                <h2 className="font-display text-2xl font-bold mb-4">Answer and steps</h2>
            ...
            {(recommendationsOnly || liveRecs.length > 0) && <aside id="recommended-tools" className={recommendationsOnly ? "order-1 w-full scroll-mt-6 space-y-4" : "scroll-mt-6 space-y-4 lg:sticky lg:top-5"}>
              <div><p className="text-xs uppercase tracking-[0.16em] font-bold text-wire">Recommended tools</p><h2 className="mt-1 font-display text-2xl font-bold">{recommendationsOnly ? 'Recommended resources' : 'Choose a tool'}</h2></div>
