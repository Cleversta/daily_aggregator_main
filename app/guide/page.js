...
export default async function GuideIndexPage() {
  ...
  return (
    <div className="space-y-12">
      ...
      <Link href="/tools/unit-converter" className="surface-card flex items-center justify-between gap-4 p-6"><div><p className="text-xs font-bold uppercase tracking-widest text-wire">Use it here · No signup</p><h2 className="mt-2 font-display text-2xl font-bold">Unit converter</h2><p className="mt-2 text-sm text-slate">Eight categories: measurements, area, speed, time, and data storage. Instant results and unit swapping.</p></div><span aria-hidden="true" className="shrink-0 text-4xl text-[#285B50]">⇄</span></Link>
      <Link href="/tools/color-picker" className="surface-card flex items-center justify-between gap-4 p-6"><div><p className="text-xs font-bold uppercase tracking-widest text-wire">Use it here · No signup</p><h2 className="mt-2 font-display text-2xl font-bold">Color picker</h2><p className="mt-2 text-sm text-slate">Pick colors, copy HEX/RGB/HSL codes, and check text contrast.</p></div><span aria-hidden="true" className="shrink-0 text-4xl text-[#754985]">◉</span></Link>
      <Link href="/tools/countdown" className="surface-card flex items-center justify-between gap-4 p-6"><div><p className="text-xs font-bold uppercase tracking-widest text-wire">Use it here · Your local time</p><h2 className="mt-2 font-display text-2xl font-bold">How long until…</h2><p className="mt-2 text-sm text-slate">Count down to holidays, special occasions, or your own date.</p></div><span aria-hidden="true" className="shrink-0 text-4xl">📅</span></Link>
      <Link href="/tools/date-calculator" className="surface-card flex items-center justify-between gap-4 p-6"><div><p className="text-xs font-bold uppercase tracking-widest text-wire">Use it here · No signup</p><h2 className="mt-2 font-display text-2xl font-bold">Date calculator</h2><p className="mt-2 text-sm text-slate">Find days between dates, add or subtract days, and calculate age.</p></div><span aria-hidden="true" className="text-4xl">📆</span></Link>
      {intents.length === 0 ? (
