...
function guideMode(value) {
  if (value === void 0 || value === null) return "ai";
  if (!contentModes.some((mode) => mode.value === value)) throw new Error("Choose a valid content mode.");
  return value;
}
__name(guideMode, "guideMode");
var categories = ["images", "documents", "coding", "money", "writing", "design", "cooking", "home"];
var starters = [
...
var str = /* @__PURE__ */ __name((value, max = 5e3) => typeof value === "string" ? value.trim().slice(0, max) : "", "str");
function normalizeGuide(input, publish = false) {
  ...
  const list = /* @__PURE__ */ __name((value, max, length) => (Array.isArray(value) ? value : []).slice(0, max).map((v) => str(v, length)).filter(Boolean), "list");
  const sources = (Array.isArray(input.sources) ? input.sources : []).slice(0, 12).map((s) => ({ title: str(s?.title, 200), url: str(s?.url, 2e3) }));
  if (sources.some((s) => !httpUrl(s.url))) throw new Error("Source links must be valid HTTP or HTTPS URLs.");
  const recommendations = (Array.isArray(input.recommendations) ? input.recommendations : []).slice(0, 5).filter((r) => r && (r.name || r.url)).map((r) => ({
    name: str(r.name, 160),
    url: str(r.url, 2e3),
    description: str(r.description),
    reason: str(r.reason),
    limitations: str(r.limitations),
    signup: str(r.signup, 300),
    privacy: str(r.privacy),
    source_urls: list(r.source_urls, 6, 2e3)
  }));
  ...
  const guide = {
    slug,
    name,
    content_mode: guideMode(input.content_mode),
    category: input.category,
    description: str(input.description, 1e3),
    phrases: list(input.phrases, 30, 150),
    steps: list(input.steps, 15, 1500),
    sources,
    recommendations,
  ...
  if (guide.verified_on && (Number.isNaN(Date.parse(guide.verified_on)) || new Date(guide.verified_on).toISOString().slice(0, 10) !== guide.verified_on || guide.verified_on > (/* @__PURE__ */ new Date()).toISOString().slice(0, 10))) throw new Error("Verification date must be a real date, no later than today.");
...
__name(normalizeGuide, "normalizeGuide");

// lib/cloudflare-access.mjs
var decode = /* @__PURE__ */ __name((value) => JSON.parse(new TextDecoder().decode(Uint8Array.from(
  atob(value.replace(/-/g, "+").replace(/_/g, "/").padEnd(Math.ceil(value.length / 4) * 4, "=")),
  (character) => character.charCodeAt(0)
))), "decode");
var teamUrl = /* @__PURE__ */ __name((value) => {
  const url = String(value || "").trim().replace(/\/$/, "");
  return url.startsWith("https://") ? url : `https://${url}`;
}, "teamUrl");
async function verifyCloudflareAccess(request, env, fetcher = fetch) {
  ...
  try {
    ...
    const jwk = keys.find((key2) => key2.kid === header.kid);
    if (!jwk) return null;
...
var unwrap = /* @__PURE__ */ __name(({ data, error }) => {
  if (error) throw new Error(error.message);
  return data;
}, "unwrap");
async function onRequestPost({ request, env, accessVerifier = verifyCloudflareAccess }) {
  let payload;
  try {
  ...
  const identity = isLocal ? { email: String(env.GUIDE_ADMIN_EMAIL || "local-admin").toLowerCase() } : await accessVerifier(request, env);
  ...
  try {
    switch (payload.action) {
      ...
      case "rebuild":
        return json(await rebuild(env));
      default:
        return json({ error: "Unknown action. Refresh the admin page after deploying the new version." }, 400);
    }
  } catch (error) {
...
async function rebuild(env) {
  ...
  try {
    const response = await fetch(env.CLOUDFLARE_DEPLOY_HOOK_URL, { method: "POST", signal: AbortSignal.timeout(15e3) });
...
__name(sendConfirmEmail, "sendConfirmEmail");
async function handleSubscribe(request, env) {
  ...
  const categories2 = Array.isArray(payload.categories) ? payload.categories : [];
  if (!EMAIL_RE.test(email)) {
    return json2({ error: "Enter a valid email address." }, 400);
  }
  const supabase = createClient(env.SUPABASE_URL, env.SUPABASE_SERVICE_ROLE_KEY, {
    auth: { persistSession: false }
  });
  const { data, error } = await supabase.from("subscribers").upsert(
    { email, categories: categories2, token: crypto.randomUUID(), confirmed_at: null, unsubscribed_at: null },
    { onConflict: "email" }
  ).select("token").single();
  if (error) {
    console.error("subscribe: upsert failed:", error.message);
    return json2({ error: "Something went wrong. Try again shortly." }, 500);
  }
  try {
    await sendConfirmEmail(env, email, data.token);
  } catch (err) {
    console.error("subscribe: confirm email failed:", err.message);
    return json2({ error: "Signed up, but the confirm email failed to send." }, 500);
  }
  return json2({ ok: true, message: "Check your inbox to confirm." });
}
...
async function handleConfirm(request, env) {
  ...
  const { data, error } = await supabase.from("subscribers").update({ confirmed_at: (/* @__PURE__ */ new Date()).toISOString() }).eq("token", token).is("unsubscribed_at", null).select("email").single();
...
// .wrangler/tmp/bundle-ZXahoK/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-ZXahoK/middleware-loader.entry.ts
