...
export const GUIDE_CATEGORIES = [
  ...
  { slug: 'cooking', title: 'Cooking', icon: '♨' },
...
export async function getIntentBySlug(slug) {
  ...
  const [{ data: recommendations }, { data: phrases }] = await Promise.all([
    supabase.from('guide_recommendations').select('*').eq('intent_id', intent.id).eq('status', 'published').order('rank'),
    supabase.from('guide_search_phrases').select('phrase').eq('intent_id', intent.id).order('created_at'),
  ]);

  return { ...intent, recommendations: recommendations || [], phrases: (phrases || []).map(row => row.phrase) };
}
