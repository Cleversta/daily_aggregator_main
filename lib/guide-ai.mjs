...
export async function researchGuide(db, job, env, fetcher = fetch) {
  ...
  if (!tavilyKey || !geminiKey || !env.GUIDE_GEMINI_MODEL) throw new Error('Configure Guide API keys and GUIDE_GEMINI_MODEL in the worker environment.');
  if (!/^[a-zA-Z0-9.-]+$/.test(env.GUIDE_GEMINI_MODEL)) throw new Error('Invalid Gemini model name.');
...
SOURCES: ${JSON.stringify(sources)}`;
  const response = await requestJSON(fetcher, `https://generativelanguage.googleapis.com/v1beta/models/${env.GUIDE_GEMINI_MODEL}:generateContent`, {
