import { httpUrl, validateAIDraft, guideMode } from './guide-workflow.mjs';
import sourceTrustModule from './source-trust.cjs';
...
export async function researchGuide(db, job, env, fetcher = fetch) {
  const mode = guideMode(job.input.content_mode);
  if (mode === 'manual') throw new Error('Manual guides do not use AI research.');
  ...
  if (!sources?.length) {
    ...
    sources = (response.results || []).filter(s => httpUrl(s.url) && sourceTrust(s.url).trusted && s.content).slice(0, 6)
...
Paraphrase and include search phrase variants.
${mode === 'recommendations' ? 'Only Recommend mode: write a short introduction and 1-3 evidence-backed recommendations. Return steps as an empty array; do not write an article.' : 'AI Info mode: write a concise answer or practical steps in the steps array. Include up to 3 recommendations only when useful to the task; otherwise return an empty recommendations array.'}
