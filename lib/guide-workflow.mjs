...
export function normalizeGuide(input, publish = false) {
  ...
  if (publish) {
    const untrusted = [...sources.map(source => source.url), ...recommendations.flatMap(rec => [rec.url, ...rec.source_urls])]
      .map(url => ({ url, result: sourceTrust(url) }))
      .find(item => !item.result.trusted);
    if (untrusted) throw new Error(`Cannot publish an untrusted link: ${untrusted.url}. ${untrusted.result.reason}`);
  }
